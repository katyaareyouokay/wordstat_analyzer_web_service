...
class SearchPhrase(Base):
    ...
    user: Mapped[Optional['User']] = relationship(
        back_populates='search_phrases')
    top_requests: Mapped[List['TopRequest']] = relationship(
        back_populates='search_phrase')
    dynamics: Mapped[List['Dynamics']] = relationship(
        back_populates='search_phrase')
    regions_requests: Mapped[List['RegionsRequest']] = relationship(
        back_populates='search_phrase')
    top_request_items: Mapped[List['TopRequestItem']] = relationship(
        back_populates='search_phrase')
    dynamics_points: Mapped[List['DynamicsPoint']] = relationship(
        back_populates='search_phrase')
    regions_requests_items: Mapped[List['RegionsRequestItem']] = relationship(
        back_populates='search_phrase')
...
class TopRequestItem(Base):
    ...
    id: Mapped[int] = mapped_column(primary_key=True)
    top_request_id: Mapped[int] = mapped_column(
        ForeignKey('top_requests.id', ondelete='CASCADE'), nullable=False)
    search_phrase_id: Mapped[Optional[int]] = mapped_column(
        ForeignKey('search_phrases.id'), nullable=True,
        comment='Связанная фраза (если присутствует в справочнике)')
    phrase: Mapped[str] = mapped_column(Text, nullable=False,
                                        comment='Текст фразы (денормализовано для быстрого доступа)')
    ...
    search_phrase: Mapped[Optional['SearchPhrase']] = relationship(
        back_populates='top_request_items')
...
class Dynamics(Base):
    ...
    group_id: Mapped[int] = mapped_column(Integer, nullable=False,
                                          comment='Идентификатор группы (повторяется для одного логического запроса с несколькими фразами)')
    search_phrase_id: Mapped[int] = mapped_column(
        ForeignKey('search_phrases.id'), nullable=False)
    requested_at: Mapped[datetime] = mapped_column(DateTime,
                                                   server_default=func.now())
    ...
    device2: Mapped[Optional['Device']] = relationship(foreign_keys=[device2_id],
                                                       back_populates='dynamics_as_device2')
    device3: Mapped[Optional['Device']] = relationship(foreign_keys=[device3_id],
                                                       back_populates='dynamics_as_device3')
    user: Mapped[Optional['User']] = relationship(back_populates='dynamics')
    points: Mapped[List['DynamicsPoint']] = relationship(
        back_populates='dynamics', cascade='all, delete-orphan')
...
class DynamicsPoint(Base):
    ...
    dynamics_id: Mapped[int] = mapped_column(
        ForeignKey('dynamics.id', ondelete='CASCADE'), nullable=False)
    search_phrase_id: Mapped[Optional[int]] = mapped_column(
        ForeignKey('search_phrases.id'), nullable=True,
        comment='Связанная фраза (если нужна дополнительная привязка)')
    ...
    search_phrase: Mapped[Optional['SearchPhrase']] = relationship(
        back_populates='dynamics_points')
...
class RegionsRequest(Base):
    ...
    requested_at: Mapped[datetime] = mapped_column(DateTime,
                                                   server_default=func.now())
    region_type: Mapped[Optional[str]] = mapped_column(String(50),
                                                       comment='Тип региона (например, "country", "city")')
    device1_id: Mapped[Optional[int]] = mapped_column(ForeignKey('devices.id'),
                                                      nullable=True)
    device2_id: Mapped[Optional[int]] = mapped_column(ForeignKey('devices.id'),
                                                      nullable=True)
    ...
    user: Mapped[Optional['User']] = relationship(
        back_populates='regions_requests')
    search_phrase: Mapped['SearchPhrase'] = relationship(
        back_populates='regions_requests')
    device1: Mapped[Optional['Device']] = relationship(foreign_keys=[device1_id],
                                                       back_populates='regions_requests_as_device1')
    device2: Mapped[Optional['Device']] = relationship(foreign_keys=[device2_id],
                                                       back_populates='regions_requests_as_device2')
    device3: Mapped[Optional['Device']] = relationship(foreign_keys=[device3_id],
                                                       back_populates='regions_requests_as_device3')
    items: Mapped[List['RegionsRequestItem']] = relationship(
        back_populates='regions_request', cascade='all, delete-orphan')
...
class RegionsRequestItem(Base):
    ...
    id: Mapped[int] = mapped_column(primary_key=True)
    regions_requests_id: Mapped[int] = mapped_column(
        ForeignKey('regions_requests.id', ondelete='CASCADE'), nullable=False)
    search_phrase_id: Mapped[int] = mapped_column(
        ForeignKey('search_phrases.id'), nullable=False)
    ...
    regions_request: Mapped['RegionsRequest'] = relationship(
        back_populates='items')
    search_phrase: Mapped['SearchPhrase'] = relationship(
        back_populates='regions_requests_items')
...
Index('idx_regions_requests_items_request',
      RegionsRequestItem.regions_requests_id)
Index('idx_regions_requests_items_search_phrase',
      RegionsRequestItem.search_phrase_id)
