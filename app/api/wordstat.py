...
from app.models import User, Dynamics, TopRequest, RegionsRequest as RegionsRequestModel
    ...
    try:
        ...
        for d in dyn_res.scalars().all():
            history_list.append({
                ...
                "created_at": d.requested_at.strftime('%Y-%m-%d %H:%M') if d.requested_at else "---"
            })

        # 2. Загружаем ТОП запросов
        top_stmt = (
            select(TopRequest)
        ...
        for t in top_res.scalars().all():
            history_list.append({
                ...
                "created_at": t.requested_at.strftime('%Y-%m-%d %H:%M') if t.requested_at else "---"
            })

        # 3. Загружаем Регионы (ИСПРАВЛЕНО: добавили 's' в RegionsRequest)
        reg_stmt = (
            select(RegionsRequestModel) # Используем переименованную модель
        ...
        for r in reg_res.scalars().all():
            history_list.append({
                ...
                "created_at": r.requested_at.strftime('%Y-%m-%d %H:%M') if r.requested_at else "---"
            })
...
async def download_excel(
    ...
    try:
        ...
        if type == "Топ запросов":
            stmt = select(TopRequestItem).where(TopRequestItem.top_request_id == item_id)
            res = await db.execute(stmt)
            items = res.scalars().all()
            data_for_excel = [{"Фраза": i.phrase, "Частота": i.count} for i in items]
            filename = f"top_requests_{item_id}.xlsx"

        # --- ЛОГИКА ДЛЯ ДИНАМИКИ ---
        elif type == "Динамика":
            stmt = select(DynamicsPoint).where(DynamicsPoint.dynamics_id == item_id)
            res = await db.execute(stmt)
            items = res.scalars().all()
            data_for_excel = [
        ...
        # --- ЛОГИКА ДЛЯ РЕГИОНОВ ---
            ...
            # Тут используем joinedload чтобы достать название региона из таблицы Region
            stmt = (
            ...
            items = res.scalars().all()
            data_for_excel = [
                {"Регион": i.region.label, "Количество": i.count, "Доля": i.share, "Affinity": i.affinity_index} 
                for i in items
            ]
